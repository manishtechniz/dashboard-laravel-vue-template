...
.tp-layout { display:grid; grid-template-columns:1fr 1.45fr; gap:18px; align-items:start; }
@media(max-width:1100px) { .tp-layout { grid-template-columns:1fr; } }

.tp-col-left, .tp-col-right { display:flex; flex-direction:column; gap:16px; }
...
/* ── Theme Cards ───────────────────────────────── */
.tp-cards-grid { display:grid; grid-template-columns:1fr 1fr; }
.tp-theme-card {
    cursor:pointer; position:relative; transition:background .15s;
    border-right:1px solid var(--border); border-bottom:1px solid var(--border);
}
.tp-theme-card:nth-child(2n) { border-right:none; }
.tp-theme-card:nth-child(n+3) { border-bottom:none; }
