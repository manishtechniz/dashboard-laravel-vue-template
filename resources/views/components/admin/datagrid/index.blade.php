...
@pushOnce('scripts')
    ...
    >
        <div class="quick-table" v-if="isQuickTable">
            <x-admin::quicktable
                {{ $attributes->only(['isPagination']) }}
                ::available="available"
                ::applied="applied"
        ...
        <div class="datagrid-table"  v-else>
            <!-- Toolbar -->
            @if ($isToolbar)
                <x-admin::datagrid.toolbar
                    {{ $attributes->only(['isPagination', 'isFilter', 'isSearchBox']) }}
                />
            @endif

            <div class="mt-4 flex">
                <x-admin::datagrid.table :isMultiRow="$isMultiRow" ::ref="'coreTable'">
                    <template #header="{
                        isLoading,
                        available,
                        applied,
    ...
    <script type="module">
        adminVueApp.component('v-datagrid', {
            ...
            methods: {
                ...
                get(extraParams = {}) {
                    ...
                    for (const column of this.applied.filters.columns) {
                        if (this.isQuickTable && column.index !== 'all') {
                            continue;
                        }
                    ...
                    this.$axios
                        ...
                        .then((response) => {
                            ...
                            // console.log("this.available.actions", this.available.actions);
                ...
                onChangePagination(newPage, newPerPage) {
                    this.applied.pagination.page = newPage;
                    this.applied.pagination.perPage = newPerPage;
                ...
                setCurrentSelectionMode() {
                    ...
                    if (! this.available.records?.length ?? false) {
                        return;
                    }
                ...
                /**
                 * Quick Table extra functionality.
                 * ---------------------------------------------------------
                 */
                getQuickSearchVales(columnIndex) {
                    let appliedColumn = this.applied.filters.columns.find(column => column.index === columnIndex);
                ...
                quickSearch($event) {
                    ...
                    let appliedColumn = this.applied.filters.columns.find(column => column.index === 'all');
