...
@pushOnce('scripts')
            ...
            <div class="flex w-full items-center gap-x-1">
                ...
                <div class="ltr:pl-2.5 rtl:pr-2.5">
                    <p class="text-sm font-light text-gray-800 dark:text-white">
                        @{{ "@lang('admin::app.components.datagrid.toolbar.length-of')".replace(':length', massActions?.indices?.length ?? 0) }}

                        @{{ "@lang('admin::app.components.datagrid.toolbar.selected')".replace(':total', available?.meta?.total ?? 0) }}
                    </p>
                </div>
            </div>
        </slot>
    ...
    <script type="module">
        adminVueApp.component('v-datagrid-mass-action', {
