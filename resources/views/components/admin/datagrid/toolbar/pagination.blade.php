...
@pushOnce('scripts')
            ...
            <template v-else>
                <div class="flex items-center gap-x-2">
                    {{-- <x-admin::form.control-group.control
                            ::value="applied.pagination.perPage"
                            type="select"
                            class="h-[38px]!"
                            ::options="available.meta.per_page_options"
                            appendTo="self"
                            @change="changePerPageOption($event.value)"
                        /> --}}
                    ...
                    <x-admin::dropdown>
                        <x-slot:toggle>
                            <button
                                type="button"
                                class="inline-flex w-full max-w-max cursor-pointer appearance-none items-center justify-between gap-x-2 rounded-md border bg-(--bg-subtle) px-2.5 py-1.5 text-center leading-6 text-(--text-muted) transition-all marker:shadow "
                            >
                                <span>
                    ...
                    <p class="whitespace-nowrap text-(--text-muted) max-sm:hidden">
                        @lang('admin::app.components.datagrid.toolbar.per-page')
                    </p>

                    <input
                        type="text"
                        class="inline-flex min-h-[38px] max-w-10 appearance-none items-center justify-center gap-x-1 rounded-md border bg-(--bg-subtle) text-(--text-muted) px-3 py-1.5 text-center leading-6   transition-all marker:shadow  focus:outline-none "
                        :value="available.meta.current_page"
                        @change="changePage(parseInt($event.target.value))"
                    >

                    <div class="whitespace-nowrap text-(--text-muted)">
                        <span>
                    ...
                    <div class="flex items-center gap-1">
                        <div
                            class="inline-flex w-full max-w-max cursor-pointer appearance-none items-center justify-between gap-x-1 rounded-md border border-transparent p-1.5 text-center text-(--text-muted) transition-all marker:shadow bg-(--bg-subtle)"
    ...
    <script type="module">
        adminVueApp.component('v-datagrid-pagination', {
